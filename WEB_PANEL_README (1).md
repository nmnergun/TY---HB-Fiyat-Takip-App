...
## Railway `start.sh not found` hatası

Bu hata Railway'in `Dockerfile` yerine Railpack kullanması veya serviste eski bir özel
başlangıç komutu bulunması halinde görülür.

1. GitHub deposunun kökünde şu dosyaların doğrudan bulunduğunu kontrol edin:
   `Dockerfile`, `railway.json`, `start.sh`, `hepsiburada_price.py`.
   Dosyalar bir ZIP'in ya da ikinci bir klasörün içinde kalmamalıdır.
2. Railway servisinde **Settings → Build → Builder** değerini **Dockerfile** seçin.
3. Aynı bölümde Dockerfile yolu sorulursa `/Dockerfile` yazın. Alternatif olarak
   Variables bölümüne `RAILWAY_DOCKERFILE_PATH=/Dockerfile` ekleyin.
4. **Settings → Deploy → Custom Start Command** alanındaki eski `start.sh` değerini
   silin. Depodaki `railway.json` doğru komutu otomatik tanımlar.
